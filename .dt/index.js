...
exports.SPECIALTY_LABELS = exports.GRADE_LABELS = exports.CITIES_BY_REGION = exports.REGION_LABELS = exports.REGIONS = exports.UNIT_OPTIONS = exports.UNIT_OPTIONS_EXTENDED = exports.SECTOR_PRODUCTS = exports.PRODUCT_TRANSLATIONS = exports.SECTOR_COLORS = exports.PRODUCT_SPECS = exports.SUB_CATEGORIES = exports.GROUP_LABELS = exports.SECTOR_LABELS = void 0;
exports.detectSubCategory = detectSubCategory;
exports.hydrateTaxonomy = hydrateTaxonomy;
exports.getDefaultUnit = getDefaultUnit;
exports.getSubCategoryLabel = getSubCategoryLabel;
exports.sortGroupKeys = sortGroupKeys;
exports.getGroupedProducts = getGroupedProducts;
exports.getProductSpecs = getProductSpecs;
exports.getGroupedSubCategories = getGroupedSubCategories;
exports.getProductLabel = getProductLabel;
exports.getUnitLabel = getUnitLabel;
exports.getRegionLabel = getRegionLabel;
const normalize_1 = require("./normalize");
// =============================================
// SECTOR LABELS (Arabic)
// =============================================
exports.SECTOR_LABELS = {
    civil: 'مدني',
...
function detectSubCategory(productName, sector) {
    const norm = (0, normalize_1.normalizeText)(productName);
    const subs = exports.SUB_CATEGORIES[sector];
    if (!subs)
        return null;
    let bestMatch = null;
    let maxScore = 0;
    for (const [key, sub] of Object.entries(subs)) {
        ...
        for (const kw of sub.keywords) {
            const nkw = (0, normalize_1.normalizeText)(kw);
            // الحارس .length>0 ضروري: norm.includes('') يُرجع true دائماً
            if (nkw.length > 0 && norm.includes(nkw))
                score++;
        }
        if (score > maxScore) {
            maxScore = score;
...
function hydrateTaxonomy(rows) {
    var _a, _b, _c, _d, _e;
    if (!Array.isArray(rows))
        return;
    for (const r of rows) {
        if (!r || !r.sector || !r.sub_key)
            continue;
        if (r.is_active === false)
            continue;
        const sec = r.sector;
        if (!exports.SUB_CATEGORIES[sec])
            exports.SUB_CATEGORIES[sec] = {};
        const ex = exports.SUB_CATEGORIES[sec][r.sub_key] || { ar: '', en: '', ur: '', icon: '📦', group: '_other', keywords: [] };
        exports.SUB_CATEGORIES[sec][r.sub_key] = {
            ar: (_a = r.name_ar) !== null && _a !== void 0 ? _a : ex.ar,
            en: (_b = r.name_en) !== null && _b !== void 0 ? _b : ex.en,
            ur: (_c = r.name_ur) !== null && _c !== void 0 ? _c : ex.ur,
            icon: (_d = r.icon) !== null && _d !== void 0 ? _d : ex.icon,
            group: (_e = r.grp) !== null && _e !== void 0 ? _e : ex.group,
...
function getDefaultUnit(productName, sector) {
    const specs = exports.PRODUCT_SPECS[(productName || '').trim()];
    if (specs) {
...
function getSubCategoryLabel(sector, subKey, locale) {
    var _a;
    const sub = (_a = exports.SUB_CATEGORIES[sector]) === null || _a === void 0 ? void 0 : _a[subKey];
    if (!sub)
        return subKey;
...
function groupRank(g) { var _a; return g === '_other' ? 9999 : ((_a = GROUP_ORDER[g]) !== null && _a !== void 0 ? _a : 900); }
...
function getGroupedProducts(sector, extra = []) {
    const subs = exports.SUB_CATEGORIES[sector] || {};
    const products = exports.SECTOR_PRODUCTS[sector] || [];
    const order = [];
    const buckets = {};
    const add = (p, hint) => {
        var _a;
        let group;
        if (hint && exports.GROUP_LABELS[hint]) {
            group = hint; // التلميح هو مفتاح مجموعة مباشرة (مواد معتمدة)
        }
        else {
            const subKey = (hint && subs[hint]) ? hint : detectSubCategory(p, sector);
            group = (subKey && ((_a = subs[subKey]) === null || _a === void 0 ? void 0 : _a.group)) || '_other';
        }
        if (!buckets[group]) {
            buckets[group] = [];
            order.push(group);
        }
        if (!buckets[group].includes(p))
    ...
    for (const e of (extra || []))
        if (e === null || e === void 0 ? void 0 : e.name)
            add(e.name, e.sub_category);
    order.sort((a, b) => groupRank(a) - groupRank(b)); // ترتيب منطقي ثابت
    return order.map(g => {
        const gl = exports.GROUP_LABELS[g];
        return {
            group: g, items: buckets[g],
            ar: (gl === null || gl === void 0 ? void 0 : gl.ar) || 'متنوّع', en: (gl === null || gl === void 0 ? void 0 : gl.en) || 'Other', ur: (gl === null || gl === void 0 ? void 0 : gl.ur) || 'متفرقہ', icon: (gl === null || gl === void 0 ? void 0 : gl.icon) || '📦',
        };
    });
}
exports.PRODUCT_SPECS = {
    'حديد تسليح': [
...
for (const g of SPEC_GROUPS) {
    for (const p of g.products)
        exports.PRODUCT_SPECS[p] = g.spec;
}
function getProductSpecs(productName) {
    return exports.PRODUCT_SPECS[(productName || '').trim()] || [];
}
function getGroupedSubCategories(sector) {
    const subs = exports.SUB_CATEGORIES[sector] || {};
    const order = [];
    const buckets = {};
    for (const [key, sub] of Object.entries(subs)) {
        const g = sub.group || '_other';
        if (!buckets[g]) {
            buckets[g] = [];
            order.push(g);
        }
    ...
    return order.map(g => {
        const gl = exports.GROUP_LABELS[g];
        return { group: g, ar: (gl === null || gl === void 0 ? void 0 : gl.ar) || 'متنوّع', en: (gl === null || gl === void 0 ? void 0 : gl.en) || 'Other', ur: (gl === null || gl === void 0 ? void 0 : gl.ur) || 'متفرقہ', icon: (gl === null || gl === void 0 ? void 0 : gl.icon) || '📦', subs: buckets[g] };
    });
}
exports.SECTOR_COLORS = {
    civil: '#dbeafe',
...
function getProductLabel(arabicName, locale) {
    if (locale === 'ar')
        return arabicName;
    const trans = exports.PRODUCT_TRANSLATIONS[arabicName];
    if (!trans)
        return arabicName;
...
function getUnitLabel(unit, locale) {
    if (!unit)
        return '';
    if (locale === 'ar')
        return unit;
...
function getRegionLabel(region, locale) {
    if (locale === 'ar')
        return region;
    return exports.REGION_LABELS[region] ? (locale === 'ur' ? exports.REGION_LABELS[region].ur : exports.REGION_LABELS[region].en) : region;
}
